...
- A first local-emulator load run (baseline vs 3 API replicas) is recorded in [LOAD_RESULTS.md](LOAD_RESULTS.md). It is single-run, 20 s per stage, generator co-located with the stack, and the raw output was not saved, so it shows relative bottlenecks only, not production capacity or an SLO. Recovery, worker scaling, per-stage CPU and AWS runs are outstanding. The deterministic stress suite covers API admission and recovery, not distributed infrastructure saturation or a soak test.
