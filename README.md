...
- Load/stress suites and an open-loop 95/5 redirect/create generator (`cmd/loadgen`, `make loadgen`) are implemented, but no saved load/scaling results are included in this directory. Prior load/stress outcomes cannot be confirmed from the retained evidence; neither suite was run during this documentation update. Sustainable throughput, saturation point, bottleneck identification, and measurable before/after scaling improvement remain pending documented runs; no production SLO is claimed. See [the results document](LOAD_RESULTS.md) for pending measurements and [the evaluation plan](DESIGN.md#10-load-test-and-scaling-plan). The deterministic stress suite covers API admission and recovery, not distributed infrastructure saturation or a long-running soak test.
